...
mkdir -p "$build"
for patch in   flexric-gcc14-kpm-v3.patch   flexric-kpm-safe-transition.patch; do
...
no_trampoline_patch="$root/integration/patches/flexric-f1c08ed-kpm-xapp-no-trampolines.patch"
git -C "$root/vendor/flexric" apply --check --ignore-space-change "$no_trampoline_patch"
git -C "$root/vendor/flexric" apply --ignore-space-change "$no_trampoline_patch"
if ! find "$build/ocudu" -type f \( -name gnb -o -name ocudu_gnb \) -perm -111   -print -quit | grep -q .; then
  cmake -S "$root/vendor/ocudu" -B "$build/ocudu" -G Ninja     -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTS=OFF     -DCMAKE_C_COMPILER_LAUNCHER=ccache -DCMAKE_CXX_COMPILER_LAUNCHER=ccache
...
sudo find "$build/flexric" -type f -name 'libkpm_sm.so'   -exec install -m 0755 {} /usr/local/lib/flexric/libkpm_sm.so \;
sudo install -m 0644 "$root/vendor/flexric/flexric.conf"   /usr/local/etc/flexric/flexric.conf
